...
public class Enemy extends ObjectImage
{
    ...
    public Enemy(Image image, double posX, double posY, Group root, double scale)
    {
        ...
        index = SceneMaker.npcM.enemies.size();

        SceneMaker.npcM.enemies.add(this);
    }
    ...
    public void animate()
    {
        ...
        SceneMaker.gc.animators.add(enemyMoving);
        enemyMoving.start();
    }
    ...
    {
        dead = true;
        SceneMaker.npcM.enemies.remove(index);

        for(Enemy en: SceneMaker.npcM.enemies)
            if(en.index > index)
                en.index--;
